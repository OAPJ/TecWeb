...
{
	$QuaryActualizar="UPDATE aulas SET Nombre='".$_POST['Nombre']."' WHERE id = ".$_POST['IdEditar'];

	mysqli_query($Link, $QuaryActualizar);
	//Evitar el reenvio del formulario
	header("Location:4EJE9_JAOP.php");
}
//Agregar
elseif(isset($_POST['Nombre']))
{
	...
	$QuaryInsercion = "INSERT INTO `aulas` (`id`,`Nombre`) VALUES (NULL, '".$varNombre."')";
...
if(isset($_GET['id']) && isset($_GET['accion']))
{
	...
	{
		$QuaryEliminar = "DELETE FROM `aulas` WHERE `aulas`.`id`=".$_GET['id'];
	...
	elseif ($_GET['accion']='editar') {
		...
		$QuaryAulas = "SELECT Nombre FROM `aulas` WHERE `aulas`.`id`=".$_GET['id'];

		$UnaAula = mysqli_query($Link, $QuaryAulas);

		$UnSoloRegistro=mysqli_fetch_assoc($UnaAula);

		$VarValorCajaAulas=$UnSoloRegistro['Nombre'];
...
$QuaryConsulta="SELECT * FROM aulas";
...
<body bgcolor="gray">
	<form method="post" action="" onsubmit="return validaFormulario();">
		...
		<table  align="center" class="table" >
			<tr>
				<td width="30%">Nombre del Aula:</td>
				<td align="LEFT" >
					<input type="text" name="Nombre" placeholder="Digite nombre del aula" id="Nombre"" value=" <?php echo $VarValorCajaAulas;?>">
				</td>
			</tr>
        ...
        while($UnSoloRegistro = mysqli_fetch_assoc($TodosLosRegistros)){
        	...
        	echo "<tr>";
                echo "<td>".$Ncomplejo."</td>";
                	$E="<a href='4EJE9_JAOP.php?id=".$id."&accion=eliminar'>X</a>";
                	$A="<a href='4EJE9_JAOP.php?id=".$id."&accion=editar'>A</a>";
                echo "<td>[".$E." | ".$A."]</td>";
            echo "</tr>";
